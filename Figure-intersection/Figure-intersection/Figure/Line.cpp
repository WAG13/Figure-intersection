...
void Line::showType() {
	...
	cout << "Points: ";
		for (int i = 0; i < getPoints().size(); i++)
			cout << "(" << getPoints()[i].first << ", " << getPoints()[i].second << ")  ";
		cout << std::endl;
	//cout << "L: (" << this->a << ", " << this->b << ", " << this->c << ") Point: (" << getX() << ", " << getY() << ")" << std::endl;
};
...
vector<Figure> Line::intersect(Circle O) {
	...
	if (d > O.getRadius()) return p;
		...
		double dest;
		dest = sqrt(pow(O.getRadius(), 2) + pow(temp.destinationPoints(O), 2));
		dest = dest / sqrt(pow(a, 2) + pow(b, 2));
		Figure p1 = temp;
